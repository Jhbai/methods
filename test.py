...
def model_kernel(conn):
    print("子進程：正在初始化模型...")
    gemma = Gemma3Object()
    conn.send("KERNEL_READY")
    while True:
        try:
            ...
            if data is None:
                break
            prompt, uid = data
            for token in gemma.predict(prompt, uid):
                conn.send(token)
            conn.send("<<EOS_SIGNAL>>")
        except EOFError:
            break
        except Exception as e:
            print(f"Kernel 發生錯誤: {e}")
            break

def model_inference(prompt, uid, conn):
    conn.send((prompt, uid))
    while True:
        try:
            res = conn.recv()
            if res == "<<EOS_SIGNAL>>":
...
if __name__ == "__main__":
    # ----- 依照Windows OS設計來設定不同process fork方式 ----- #
    # multiprocessing.set_start_method('spawn', force=True)
    
    parent_conn, child_conn = multiprocessing.Pipe()
    
    p = multiprocessing.Process(target=model_kernel, args=(child_conn,))
    p.daemon = True
    p.start()
    ...
    # ----- 實際傳入一段promt，等待LLM回傳，並印出結果 ----- #
    test_prompt = "幫我寫一段 Python 裝飾器程式碼"
    test_uid = "bear_tsmc_it"
    
    print(f"熊熊，模型開始回傳：\n")
    for token in model_inference(test_prompt, test_uid, parent_conn):
