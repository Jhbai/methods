...
class Gemma3Object:
    ...
    def decision(self, prompt: str):
        ...
        # ----- Prefill ----- #
        chunks = torch.split(input_ids[:, :-1], 256, dim=-1)
        st = 0
        ed = 0
        with torch.no_grad():
            for chunk in chunks:
    ...
    def function_call(self, prompt: str, tools: list):
        ...
        full_prompt += "目前問題是{prompt}，請根據的需求選擇最合適的工具並生成相應的函數調用。\n請以JSON格式返回函數調用，包含工具名稱和參數。格式為 \{\"tool_name\": 工具名稱, \"parameters\": 參數\}"
        return self.chat(full_prompt, uid="function_call", force="{\"tool_name\":")
    ...
    def chat(self, prompt: str, uid: str, force: str = ""):
        ...
        # ----- Prefill ----- #
        print(f"prefilling")
        chunks = torch.split(input_ids[:, :-1], 512, dim=-1)
        st = 0
        ed = 0
