...
@register_chat_completion_handler("qwen-agent")
def qwen_agent_chat_handler(
    llama: llama.Llama,
    messages: List[llama_types.ChatCompletionRequestMessage],
    functions: Optional[List[llama_types.ChatCompletionFunction]] = None,
    function_call: Optional[llama_types.ChatCompletionRequestFunctionCall] = None,
    ...
    **kwargs, 
    ...
    # 1. 準備 Tools (將 tools 轉為 function definitions)
    # Qwen 喜歡我們把工具定義直接寫在 System Prompt 裡
    tools_definitions = []
    if tools:
        for tool in tools:
            if tool["type"] == "function":
                tools_definitions.append(tool["function"])
    
    # 2. 建構 Prompt
    # 我們使用標準 ChatML 格式，但會注入工具定義
    system_message = _get_system_message(messages)
    
    # 如果有工具，將其注入到 system prompt
    if tools_definitions:
        tools_json = json.dumps(tools_definitions, indent=2)
        system_injection = (
            f"\n\nYou have access to the following tools:\n{tools_json}\n\n"
            "If you need to call a tool, please output the call in the following XML format:\n"
            "<function_call>\n"
            "{\"name\": \"tool_name\", \"arguments\": {\"param\": \"value\"}}\n"
            "</function_call>\n"
        )
        # 如果原本沒有 system prompt，就創造一個；如果有，就接在後面
        if system_message:
            # 替換掉原本的 system message 內容 (因為 messages 是唯讀的，我們要用替換法)
            new_messages = []
            for msg in messages:
                if msg["role"] == "system":
                    new_messages.append({
            ...
            # 如果沒有 system prompt，插入一個在最前面
            messages = [{
                "role": "system", 
                "content": "You are a helpful assistant." + system_injection
            }] + messages

    # 使用 ChatML 模板渲染 Prompt
    # 這裡我們直接借用 chatml 的邏輯，但手動處理
    prompt = ""
    for msg in messages:
        role = msg["role"]
        content = msg["content"]
        if content is None: continue
        # 處理 content 為 list 的情況 (多模態) - 沿用我們之前的修復
    ...
    completion = llama.create_completion(
        ...
        typical_p=typical_p,
        stream=stream,
        stop=["<|im_end|>", "</s>"] + (stop if stop else []),
        max_tokens=max_tokens,
        presence_penalty=presence_penalty,
        frequency_penalty=frequency_penalty,
        repeat_penalty=repeat_penalty,
    ...
    # 4. 解析結果 (Server Side Parsing)
    if stream:
        # TODO: Streaming 模式下如果要支援 Tool Call 解析會比較複雜
        # 暫時回傳原始 Stream，通常 Agent 會自己處理 Stream 中的文字
        return _convert_completion_to_chat(completion, stream=True)
    else:
        text_content = completion["choices"][0]["text"]
        
        # 使用 Regex 抓取 <function_call>
        # Qwen 有時候會輸出 ```xml <function_call>...```，所以 regex 要寬容一點
        tool_calls = []
        match = re.search(r"<function_call>\s*({.*?})\s*</function_call>", text_content, re.DOTALL)
        ...
        if match and tools:
            try:
                ...
                # 建構標準 OpenAI tool_calls 物件
                tool_call_id = "call_" + completion["id"]
                tool_calls.append({
                    "id": tool_call_id,
                    "type": "function",
                    "function": {
                        "name": function_args.get("name"),
                        "arguments": json.dumps(function_args.get("arguments", {}))
                    }
                })
                
                # 如果成功解析出工具，content 應該設為 null，並填入 tool_calls
                return {
                    "id": "chat" + completion["id"],
                    "object": "chat.completion",
                    "created": completion["created"],
                    ...
                    "choices": [{
                        ...
                        "message": {
                            "role": "assistant",
                            "content": None,  # 清空 content
                            "tool_calls": tool_calls, # 填入 tool_calls
                        },
                        "logprobs": completion["choices"][0].get("logprobs"),
                        "finish_reason": "tool_calls" # 修改 finish_reason
                    }],
                    "usage": completion["usage"],
                }
            except Exception as e:
                print(f"Failed to parse function call JSON: {e}")
                # 解析失敗就退化回傳原始文字
                pass

        # 如果沒有匹配到工具，或者解析失敗，就回傳原始回應
